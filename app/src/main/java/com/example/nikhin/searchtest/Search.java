...
public class Search {
	...
	public static void inputData(String filename, Context context) {
		int i, j;
		//File textFile = new File("mnt/sdcard/input1.txt");
		BufferedReader bufferedReader;

		boolean isInvalidInput = false;
		try {
			//input from File
			//FileInputStream fis = new FileInputStream(textFile); //this is for file of localstorage - sdcard
			final InputStream fis = context.getAssets().open(filename);
			...
			for (i = 0; i < lines.size(); i++) {
				String[] strings = lines.get(i).toString().split(" ");
